...
function p(a) {
  ...
  if (g-- > 0)
    for (
      h < this.DB && (c = this[g] >> h) > 0 && ((e = !0), (f = i(c)));
      g >= 0;
...
function C(a, b, c) {
  ...
  if (!(f.t <= 0)) {
    ...
    if (0 != m) {
      ...
      for (
        h.dlShiftTo(s, u),
          c.compareTo(u) >= 0 && ((c[c.t++] = 1), c.subTo(u, c)),
          d.ONE.dlShiftTo(l, u),
          u.subTo(h, h);
        h.t < l;
      ...
      for (; --s >= 0; ) {
        var v =
          c[--r] == m ? this.DM : Math.floor(c[r] * o + (c[r - 1] + q) * p);
...
function eb(a, b) {
  null != a && null != b && a.length > 0 && b.length > 0
    ? ((this.n = bb(a, 16)), (this.e = parseInt(b, 16)))
    : alert("ÍøÂçÒì³££¬Çëµã»÷µÇÂ¼ÖØÊÔ");
}
function fb(a) {
...
jb && "Microsoft Internet Explorer" == navigator.appName
  ? ((d.prototype.am = g), (hb = 30))
  : jb && "Netscape" != navigator.appName
  ? ((d.prototype.am = f), (hb = 26))
  : ((d.prototype.am = h), (hb = 28)),
  (d.prototype.DB = hb),
...
var lb,
  mb,
  nb = "0123456789abcdefghijklmnopqrstuvwxyz",
  ob = new Array();
...
let target = {
  ...
  fingerPrintDetail: fp.details,
};
console.info(target);

// let obj = Object.assign(source, target);
...
//   try {
//     const url = "https://passport.migu.cn/authn";
